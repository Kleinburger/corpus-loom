...
class Store:
    ...
    def new_conversation(self, name: Optional[str], system: Optional[str]) -> str:
        ...
        now = time.time()
        with self._lock, self._conn() as con:
            con.execute("INSERT INTO conversations(id, name, system, created_at) VALUES (?, ?, ?, ?)",
                        (cid, name, system, now))
        return cid

    def append_message(self, convo_id: str, role: str, content: str, meta: Optional[Dict[str, Any]] = None) -> int:
        meta_json = json.dumps(meta or {})
        now = time.time()
        with self._lock, self._conn() as con:
            row = con.execute("SELECT COALESCE(MAX(seq), -1) AS m FROM messages WHERE convo_id=?", (convo_id,)).fetchone()
    ...
    def get_history(self, convo_id: str) -> List[Message]:
        with self._lock, self._conn() as con:
            rows = con.execute("SELECT role, content FROM messages WHERE convo_id=? ORDER BY seq ASC", (convo_id,)).fetchall()
    ...
    def upsert_document(self, source: str, meta: Optional[Dict[str, Any]] = None) -> str:
        ...
        now = time.time()
        with self._lock, self._conn() as con:
            con.execute("INSERT INTO documents(id, source, meta_json, created_at) VALUES (?, ?, ?, ?)",
                        (did, source, json.dumps(meta or {}), now))
        return did

    def insert_chunk(self, doc_id: str, idx: int, text: str, vector: Optional[List[float]], meta: Optional[Dict[str, Any]]):
        cid = str(uuid.uuid4())
        now = time.time()
        with self._lock, self._conn() as con:
            con.execute(
                ...
                VALUES(?, ?, ?, ?, ?, ?, ?)
                """, (cid, doc_id, idx, text, json.dumps(vector) if vector is not None else None,
                        json.dumps(meta or {}), now))
        return cid

    def iter_chunks(self) -> Iterable[sqlite3.Row]:
        with self._lock, self._conn() as con:
            for row in con.execute("SELECT id, doc_id, idx, text, vector_json, meta_json FROM chunks"):
                yield row

    def update_chunk_vector(self, chunk_id: str, vector: List[float]) -> None:
        with self._lock, self._conn() as con:
            con.execute("UPDATE chunks SET vector_json=? WHERE id=?", (json.dumps(vector), chunk_id))

    # helpers for re-ingest/versioning
    def get_latest_document_by_source(self, source: str):
        with self._lock, self._conn() as con:
            row = con.execute("SELECT id, meta_json FROM documents WHERE source=? ORDER BY created_at DESC LIMIT 1", (source,)).fetchone()
        if not row:
            return None
    ...
    def update_document_meta(self, doc_id: str, meta: dict) -> None:
        with self._lock, self._conn() as con:
            con.execute("UPDATE documents SET meta_json=? WHERE id=?", (json.dumps(meta), doc_id))
