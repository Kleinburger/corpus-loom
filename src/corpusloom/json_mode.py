...
class JsonMode:
    def __init__(self, post_func, get_history_func, append_message_func, get_system_func, model: str, keep_alive: Optional[str], default_options: Dict[str, Any]):
    ...
    def _ensure_pydantic(self):
        if not _PYDANTIC_AVAILABLE:
            raise RuntimeError("Install pydantic to use JSON validation: pip install 'pydantic>=1.10'")
    ...
    def generate_json(self, prompt: str, schema: Type["BaseModel"], options: Optional[Dict[str, Any]] = None, retries: int = 2):
        ...
        for _ in range(retries + 1):
            res = self._post("/api/chat", {"model": self.model, "messages": messages, "options": opts, "keep_alive": self.keep_alive, "stream": False})
            candidate = res.get("message", {}).get("content", "")
            last_raw = candidate
            jtxt = extract_json_str(candidate)
            if not jtxt:
                messages += [{"role": "assistant", "content": candidate}, {"role": "user", "content": "Return ONLY valid JSON."}]
                continue
            try:
    ...
    def chat_json(self, convo_id: str, user_message: str, schema: Type["BaseModel"], options: Optional[Dict[str, Any]] = None, retries: int = 2):
        self._ensure_pydantic()
        system = self._get_system(convo_id) or ""
        sys_combo = (JSON_SYSTEM + "\n\n" + system).strip()
        self._append(convo_id, "user", user_message, {"kind": "chat_json_input"})
        msgs = [{"role": "system", "content": sys_combo}] + [{"role": m.role, "content": m.content} for m in self._get_history(convo_id)]
        opts = {**self.default_options, **(options or {}), "temperature": 0.0}
        last_raw = ""
        for _ in range(retries + 1):
            res = self._post("/api/chat", {"model": self.model, "messages": msgs, "options": opts, "keep_alive": self.keep_alive, "stream": False})
            candidate = res.get("message", {}).get("content", "")
            last_raw = candidate
            jtxt = extract_json_str(candidate)
            if not jtxt:
                msgs += [{"role": "assistant", "content": candidate}, {"role": "user", "content": "Return ONLY valid JSON."}]
                continue
            try:
